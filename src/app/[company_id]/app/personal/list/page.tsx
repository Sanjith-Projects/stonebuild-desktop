...
type LoanDetails = Loan; // No collateral/jewels for personal loan
...
export default function LoanList() {
  ...
  const applyFilter = async () => {
    ...
    setLoading(true);
    try {
      const res = await postRequest({ token: "getPersonalLoan", data: dataPayload });
  ...
  const fetchLoans = async () => {
    setLoading(true);
    try {
      const res = await postRequest({ token: "getPersonalLoan" });
      if (res.success && Array.isArray(res.data)) {
        setLoanList(res.data);
      } else {
        setLoanList([]);
  ...
  // fetchLoanDetails now expects no collateral — reasonable fallback handling
  const fetchLoanDetails = async (loan: Loan) => {
    setDetailsLoading(true);
    setSelectedLoan({ ...loan });
    try {
      // ask backend for loan data (may return object or array)
      const res = await postRequest({ token: "getPersonalLoan", data: { id: loan.id } });
      ...
      if (res.success) {
        ...
        // If array returned, try to find matching id else use first row to enrich
        if (Array.isArray(d) && d.length > 0) {
          const found = d.find((row: any) => String(row.id) === String(loan.id) || String(row.loan_id) === String(loan.loan_id));
          const detailRow = found || d[0];
          setSelectedLoan({
            ...loan,
            ...detailRow,
          });
        ...
        setSelectedLoan({ ...loan });
        setViewOpen(true);
        toast("No additional loan details returned; showing basic data", { icon: "ℹ️" });
      } else {
        setSelectedLoan({ ...loan });
        setViewOpen(true);
        toast(res.message || "Failed to load loan details", { icon: "⚠️" });
      }
    } catch (err) {
  ...
  const handleStatusUpdate = async () => {
    ...
    if (newStatus === "approved" && !jewelleryReceived) {
      toast.error("Please confirm that the customer has received the amount.");
      return;
    }
    const payload = {
    ...
    setDetailsLoading(true);
    try {
      const res = await postRequest({ token: "updatePersonalLoanStatus", data: payload });
      if (res.success) {
        setLoanList((prevList) =>
          prevList.map((loan) =>
            loan.id === loanToUpdate.id ? { ...loan, status: newStatus as Loan["status"] } : loan
          )
        );
  ...
  return (
    <div className="bg-white p-6">
        ...
        {detailsLoading ? (
          ...
          <div id="printArea" className="space-y-6 text-sm text-gray-800 print:p-8 print:bg-white p-5">
            ...
            <div className="text-center mt-2">
              <h3 className="text-lg font-semibold underline">Personal Loan Receipt</h3>
            </div>
            <div className="grid grid-cols-2 gap-4 border p-4 rounded-lg bg-gray-50">
              ...
              <div>
                ...
                <p>{selectedLoan.interest_percent}% p.a.</p>
              </div>
              <div>
            ...
            <div className="mt-6 border-t pt-4 text-xs text-gray-600 leading-relaxed">
              ...
              <ul className="list-disc ml-4 space-y-1">
                ...
                <li>In case of default, the company reserves the right to take necessary recovery actions.</li>
      ...
      <Modal
        ...
        title={`Update Status for Loan: ${loanToUpdate?.id}`}
        ...
        <div className="space-y-4 p-4">
          ...
          {updateState.newStatus === "approved" && (
            <div className="flex items-center pt-2">
              ...
              <label htmlFor="jewellery-received" className="ml-2 block  text-[16px] text-gray-900 font-medium">Confirm: Customer has received the amount.</label>
            </div>
          )}
